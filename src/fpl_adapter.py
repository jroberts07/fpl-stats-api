...
from database_adapter import (
    put_local_league_data, put_local_static_data, put_local_entry_team_data
)
...
async def get_remote_league_data(league_id, player_cookie, app):
    """Inner function responsible for gathering remote league data processing
...
async def get_remote_entry_team_data(
        entry_id, gameweek, player_cookie, app
):
    """Function responsible for gathering the entries team for this gameweek
    remotely

    Args:
        entry_id (int): The ID of the entry's team to fetch.
        gameweek (int): The gameweek to fetch the team for.
        player_cookie (obj): The player cookie used for authentication on
        the FPL API.
        app (obj): The sanic app.

    Returns:
        obj: The entries team for that gameweek
    """
    logger.info('Fetching remote entry players data')
    url = app.config.FPL_URL + app.config.ENTRY_TEAM_DATA.format(
        entry_id=entry_id, gameweek=gameweek
    )
    remote_entry_team_data = await call_fpl_endpoint(
        url, player_cookie, app.config
    )
    try:
        await put_local_entry_team_data(
            app.db, remote_entry_team_data, gameweek
        )
    except Exception as e:
        # Log the error but we don't care if we cant save locally.
        logger.exception(e)
    return remote_entry_team_data
